...
class GenerateFEMM_IPMSM:
    ...
    def populateFunctionLuaScript(self, save_dir):
        lua_path = "backend/test_design/topology/femm/SlidingBand.lua"
        ...
        # Call FEMM to execute the Lua script
        femm.callfemm('dofile("' + lua_path + '")')
        ...
        with open("{}/Sliding_data.txt".format(save_dir), "r") as f:
            s = f.read().split("\n")
    ...
    def make_plot_sliding_band(self, NumPoints=1440):
        ...
        b_arr, angle_value = self.populateFunctionLuaScript(self.directory)    
    ...
    def calculate_LDLQ(self, currentPk, current_angle, pole_pairs,rotor_angle=0):
        ...
        # wd0,wq0,t = -0.004868758566247777,0.07257919599253183,-1
        
        wd,wq,torque = self.get_fluxlinkage(fname,ia,ib,ic,rotor_angle,pole_pairs)
        # wd,wq,torque = -0.01124435713937827,0.07151969522927708,-1
