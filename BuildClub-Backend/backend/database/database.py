...
#connection_str = "mongodb://3.109.133.249:27017/"
...
def delete_application(user_id, application_name):
    try:
        ...
        if result.deleted_count == 1:
            motors_collection.delete_many({"user_id": ObjectId(user_id), "vehicle_name": application_name})
...
def get_all_users():
    try:
        users = users_collection.find({}, {"_id": 1,"user_name": 1, "email": 1, "valid": 1})
        user_list = []
        for user in users:
            user_list.append({
                "id": str(user["_id"]),
...
def update_user_status(user_id, new_status):
    try:
        # Update user status in MongoDB collection
        users_collection.update_one(
           {"_id": ObjectId(user_id)},
            {'$set': {'valid': new_status}}
        )
...
def get_user_statistics():
    try:
        active_user_count = users_collection.count_documents({"valid": True})
        not_active_user_count = users_collection.count_documents({"valid": False})
        active_non_pilabz_user_count = users_collection.count_documents({
            "valid": True,
            "email": {"$not": {"$regex": ".*@pilabz\.in$"}}
        })
        return {
            "activeUserCount": active_user_count,
            "notActiveUserCount": not_active_user_count,
            "activeNonPilabzUserCount": active_non_pilabz_user_count
        }, 200
    except Exception as e:
        return {"error": str(e)}, 500

def generate_user_list_csv():
    try:
